...
interface QuoteRequestFormProps {
  ...
  preferredRequestType?: "purchase" | "rental" | "maintenance" | "fit-test" | "mixed"
  sourcePage?: string
  contextLabel?: string
...
export function QuoteRequestForm({
  ...
  const requestTypeLabel =
    preferredRequestType === "purchase"
      ? "Achat"
      : preferredRequestType === "rental"
        ? "Location"
        : preferredRequestType === "maintenance"
          ? "Maintenance"
          : preferredRequestType === "fit-test"
            ? "FIT TEST"
            : "Mixte"
  ...
  React.useEffect(() => {
    safeTrack("Quote Form Viewed", {
      ...
      request_type: preferredRequestType,
      with_product: Boolean(requestedProduct),
      with_cart: includeCart && items.length > 0,
    })
  }, [contextLabel, includeCart, items.length, preferredRequestType, requestedProduct, sourcePage])
          ...
          <CardContent className="space-y-6 p-6 lg:p-8">
            <div>
              ...
              <p className="mt-2 text-sm leading-6 text-muted-foreground">
                La demande est maintenant connectée au back-office Epicap. Elle est créée dans la
                base avec une référence exploitable côté commerce.
              </p>
            </div>
            ...
            <form action={formAction} className="space-y-6">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <Field label="Société" htmlFor="companyName">
                  <Input
                    ...
                    placeholder="Entreprise de désamiantage"
                    required
                  />
                </Field>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label="Type de demande" htmlFor="requestType">
                  <select
                    id="requestType"
                    name="requestType"
                    defaultValue={preferredRequestType}
                ...
                <Field label="Agence / zone" htmlFor="requestedAgency">
                  <Input
                    ...
                    placeholder="Île-de-France, Normandie, multi-sites..."
                  />
                </Field>
                ...
                <Field label="Durée location (jours)" htmlFor="rentalDays">
                  <Input
                    id="rentalDays"
                    name="rentalDays"
                    type="number"
              ...
              {requestedProduct ? (
                <div className="rounded-[1.2rem] border border-border/70 bg-muted/20 p-4">
                  ...
                  <div className="mt-4 max-w-[180px]">
                    <Field label="Quantité" htmlFor="productQuantity" compact>
                      <Input
                        id="productQuantity"
              ...
              <Field label="Contexte chantier et besoin" htmlFor="message">
                <Textarea
                  id="message"
                  name="message"
                  placeholder="Nature du chantier, environnement amiante/plomb, délais, arbitrage achat/location, contraintes d'accès, besoin de maintenance ou FIT TEST..."
                  className="min-h-36"
                  required
                />
              </Field>
        ...
        <div className="space-y-6">
          <Card className="p-0">
            <CardContent className="space-y-4 p-6">
              ...
              <InfoRow
                label="Compte"
                value={user ? "Identifié" : "Public non connecté"}
              />
            </CardContent>
          </Card>
          ...
          <Card className="p-0">
            <CardContent className="space-y-4 p-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-base font-semibold">B2B Epicap</p>
                  <p className="text-sm text-muted-foreground">
                    Les conditions commerciales sont maintenant prises en compte dans les calculs.
                  </p>
                </div>
              </div>
              <InfoRow
                label="Société"
                value={defaultCompanyName || "À renseigner dans le formulaire"}
              />
              <InfoRow
                label="Remise société"
                value={companyDiscount > 0 ? `${companyDiscount.toFixed(0)}%` : "Aucune"}
              />
              <InfoRow
                label="Panier joint"
          ...
          <Card className="border-primary/20 bg-primary/6 p-0">
            <CardContent className="space-y-3 p-6">
              ...
              <p className="text-sm leading-6 text-muted-foreground">
                Chaque demande crée maintenant une référence exploitable côté dashboard. Le besoin
                n’est plus perdu dans un simple lien email.
              </p>
